...
def report(records: list[dict[str, Any]], kind: str) -> None:
    ...
    print("components=" + ", ".join(f"{name}:{count}" for name, count in sorted(component_counts.items())))
    print("events=" + ", ".join(f"{name}:{count}" for name, count in sorted(event_counts.items())))
