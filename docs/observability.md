...
> 原始基线，无 TopK / 无 make_budget，全量扫描后排序。
...
>
> `s` 的 trunc%=100% 是预期行为 — 单字母输入触发 `make_budget(1,7)` 的 scan 上限。

### Phase 2 vs Phase 1 结论

| 指标 | 结论 |
|------|------|
| 功能正确性 | ✅ 路径数、候选数与 Phase 1 完全一致，无回归 |
| 单字母 `s` | 查询 P50 从 785μs 降至 94μs（**-88%**），TopK 截断了大量无用扫描 |
| 双字母 `bj` | e2e P50 从 2067μs 降至 210μs（**-90%**），TopK + make_budget 收益最大 |
| 缩写无匹配（sdf/shrf 等） | e2e 延迟下降 10%–29%，Syllabifier 路径枚举仍是主要开销 |
| 全拼（nihao） | e2e P50 从 3814μs 降至 2494μs（-35%），候选质量无变化 |
| 查询 P99 稳定性 | 个别输入（shrf/nihaoshijie）P99 波动 ±15%，属正常测量噪声 |
| truncated 触发 | 仅 `s`（单字母）触发 100%，符合 make_budget 分档预期 |

**一句话总结：** Phase 2 在保持候选结果一致的前提下，短输入（1–2 字母）查询延迟下降 64%–90%，长输入无劣化。
