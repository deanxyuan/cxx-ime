...
面板顶部提供"用户词库 / 选词偏好"资源切换（Tab）与"拼音 / 五笔"下拉框。资源决定操作对象：用户词库对应手工添加的词条，选词偏好对应候选学习记录；切换后下方的查询结果、管理、导入导出和状态栏均针对当前资源与词典。

状态栏显示当前数据文件路径（用户词库如 `%USERPROFILE%\cxxime\user_pinyin.tsv`，选词偏好如 `%USERPROFILE%\cxxime\learning_pinyin.tsv`），鼠标悬停可查看完整路径。
...
在"用户词库"资源下手动添加、保存或删除词条。操作后，面板顶部状态栏会即时反馈操作结果（如"已新增词条，列表已刷新""已保存修改，列表已刷新""已删除词条，列表已刷新"）。需要输入法服务端正在运行（通过 IPC 通信）。

切换至"选词偏好"资源后，编辑区隐藏，可点击"清空当前偏好"按钮清除当前词典（拼音或五笔）的全部学习记录；偏好仅用于本机排序，不影响用户词库。

1. 在"词语"框输入要添加的词（如 `你好世界`）
2. 在"编码"框输入对应的编码（如 `nihaoshijie`）
3. 点击"新增"按钮

添加成功后词条立即生效，写入对应的用户词库文件（`user_pinyin.tsv` / `user_wubi.tsv`）。如果服务端未运行，会弹出连接失败提示。

### 导入与导出

- **导入**（仅用户词库）：选择 `.tsv` 文件后，面板顶部状态栏提示"导入完成，列表已刷新"。若服务未能立即重新加载，会弹出警告提示"文件已导入但服务未能立即重新加载"。
