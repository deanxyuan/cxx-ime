...
控制选词后是否自动记录候选偏好。启用后选中的候选会作为独立的偏好数据保存（不写入用户词库），仅用于本地候选排序；可在"词库管理"面板的"选词偏好"页中查看和清空。
...
面板顶部提供"词条 / 选词偏好"两个视图切换按钮与"拼音 / 五笔"下拉框。视图决定操作对象："词条"页统一查询系统与用户来源的词条，并管理用户词和系统词停用状态；"选词偏好"对应候选学习记录。切换后下方的查询结果、管理、导入导出和状态栏均针对当前视图与词典。
