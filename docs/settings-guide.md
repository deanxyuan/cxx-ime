...
CxxIME 设置窗口（`cxxime-settings.exe`）是输入法的图形化配置工具，采用 Win32 原生控件构建。窗口左侧为导航栏，右侧为对应设置面板，底部提供"确定"、"取消"、"应用"三个按钮。

- **确定**：保存配置并关闭窗口
- **取消**：不保存直接关闭
- **应用**：保存配置但不关闭窗口，方便继续调整

配置文件保存位置：`%APPDATA%\CxxIME\default.json`
...
**配置项**：`preedit_type`（枚举：`composition` / `preview`）
...
**配置项**：`page_size`（整数，默认 9）

每页显示的候选词数量，范围建议 5–15。设置后按翻页键可查看超出部分。

### 五笔四码上屏

**配置项**：`wubi_auto_commit_4code`（布尔值）
...
**配置项**：`font_name`（字符串，默认 `Microsoft YaHei UI`）

点击 `...` 按钮打开系统字体选择对话框。可同时修改字体名称和字号。

### 候选字号

**配置项**：`font_size`（整数，默认 14，最小 8）

候选词的显示字号（磅值）。与字体选择器联动——在字体对话框中修改字号后自动同步。

### 编码字号

**配置项**：`layout_config.label_font_point`（整数，默认 0）
...
**配置项**：`layout`（字符串：`horizontal` / `vertical`）
...
**配置项**：`render_backend`（字符串：`d2d` / `gdi`）
...
| 参数 | 配置项 | 默认值 | 说明 |
|------|--------|--------|------|
| 最小宽度 | `layout_config.min_width` | 160 | 候选窗口最小宽度（像素） |
| 最大宽度 | `layout_config.max_width` | 0 | 最大宽度，0 表示不限制 |
| 最大高度 | `layout_config.max_height` | 0 | 最大高度，0 表示不限制 |

### 边距和间距

| 参数 | 配置项 | 默认值 | 说明 |
|------|--------|--------|------|
| 水平边距 | `layout_config.margin_x` | 12 | 窗口内容区左右内边距 |
| 垂直边距 | `layout_config.margin_y` | 12 | 窗口内容区上下内边距 |
| 间距 | `layout_config.spacing` | 10 | 编码区与候选区之间的间距 |
| 候选间距 | `layout_config.candidate_spacing` | 8 | 候选词之间的间距 |

### 高亮区域

| 参数 | 配置项 | 默认值 | 说明 |
|------|--------|--------|------|
| 高亮内边距X | `layout_config.hilite_padding_x` | 4 | 高亮矩形水平内边距 |
| 高亮内边距Y | `layout_config.hilite_padding_y` | 2 | 高亮矩形垂直内边距 |
| 高亮间距 | `layout_config.hilite_spacing` | 4 | 高亮区域内标签与文字的间距 |
| 圆角半径 | `layout_config.round_corner` | 4 | 高亮矩形的圆角半径 |

### 窗口外观

| 参数 | 配置项 | 默认值 | 说明 |
|------|--------|--------|------|
| 窗口圆角 | `layout_config.round_corner_ex` | 4 | 候选窗口整体的圆角半径 |
| 边框宽度 | `layout_config.border_width` | 1 | 候选窗口边框宽度 |
...
| `noop` | 不处理，Shift 仅作为修饰键 |

左右 Shift 键可分别配置不同行为。

### Control 键行为

可选项与 Shift 相同：`inline_ascii`、`code`、`candidate`、`clear`、`noop`。

左右 Control 键可分别配置。

### Caps Lock 行为

**配置项**：`ascii_switch_key.Caps_Lock`
...
> **注**：CapsLock 模式仅影响 A-Z 字母键。数字键、标点键在所有模式下行为一致。

### 传统 Caps Lock

**配置项**：`good_old_caps_lock`（布尔值）

启用后，CapsLock 恢复传统的大小写切换功能（不触发中英文切换）。适合习惯用 CapsLock 切换大小写的用户。
...
| 文件 | 说明 |
|------|------|
| `pinyin.dict.bin` | 拼音二进制词典（mmap 格式，由 SQLite 源词典构建） |
| `wubi86.dict.bin` | 五笔 86 版二进制词典 |
| `user.tsv` | 用户自定义词典（TSV 格式，自动学习 + 手动添加） |

### 快捷造词

手动添加用户词条。需要输入法服务端正在运行（通过 IPC 通信）。

1. 在"词语"框输入要添加的词（如 `你好世界`）
2. 在"编码"框输入对应的编码（如 `nihaoshijie`）
3. 点击"添加"按钮

添加成功后词条立即生效，写入 `user.tsv`。如果服务端未运行，会提示连接失败。
...
2. `%APPDATA%\CxxIME\default.json`（用户自定义，覆盖同名字段）
...
2. 保存到 `%APPDATA%\CxxIME\default.json`
