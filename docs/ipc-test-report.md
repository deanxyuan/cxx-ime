...
日期：2026-05-24
最后更新：用户词典 SQLite→内存优化完成后（阶段 3）

## 架构变更历程

### 阶段 1：IPC 传输层 IOCP 重新设计

详见 `docs/ipc-high-performance-redesign.md`。
...
详见 `docs/engine-shared-resources-preload.md`。

Dict、SpellingsIndex、Config 在 server 启动时加载一次，所有 session 共享引用。session 创建不再有文件 I/O。

### 阶段 3：用户词典 SQLite → 内存

详见 `docs/user-dict-sqlite-to-memory.md`。
...
**IPC**（9）：`start_session`、`end_session`、`process_key_preedit`、`process_key_commit`、`process_key_rejected`、`select_candidate`、`commit_composition`、`focus_in_out`、`send_request`
...
2. ~~**bench 延迟偏高**~~ — 已解决。IPC 传输延迟从 ~15ms 降至 ~50us（preedit avg），提升 294x。

3. ~~**pinyin buffer 填满后延迟递增**~~ — 已解决。Syllabifier 路径枚举加入 10000 上限 + 信度排序。

4. ~~**并发 client 连接偶尔超时**~~ — 已解决。`IpcClient::connect` 加入重试循环。
