...
TEST(Shuangpin, built_in_scheme_descriptors_are_stable) {
    ...
    ASSERT_EQ(schemes.size(), static_cast<std::size_t>(2));
    ASSERT_EQ(std::string(schemes[0].id), "full_pinyin");
    ASSERT_EQ(std::string(schemes[1].id), "microsoft_shuangpin");
    ASSERT_EQ(schemes[1].kind, cxxime::PinyinSchemeKind::kShuangpin);
    ASSERT_EQ(std::string(schemes[1].manifest_role), "pinyin_spellings_microsoft_shuangpin");
    ASSERT_EQ(std::string(schemes[1].spelling_filename),
              "pinyin.microsoft-shuangpin.spellings.bin");
    ASSERT_EQ(std::wstring(schemes[1].nihao_preedit_example), L"ni'hk");
    ASSERT_EQ(std::string(schemes[1].wuzong_preedit_example), "wu'zs");
    ASSERT_EQ(std::string(schemes[1].jishu_preedit_example), "ji'uu");
...
TEST(Shuangpin, built_in_scheme_descriptors_are_complete_and_unique) {
    ...
    for (const auto& scheme : cxxime::built_in_pinyin_schemes()) {
        ...
        ASSERT_TRUE(scheme.nihao_preedit_example && *scheme.nihao_preedit_example);
        ASSERT_TRUE(scheme.wuzong_preedit_example && *scheme.wuzong_preedit_example);
        ASSERT_TRUE(scheme.jishu_preedit_example && *scheme.jishu_preedit_example);
